...
impl Parser {
    ...
    fn where_clause(&mut self) -> Result<WhereClause, String> {
        let left = if self.check(SQLTokenTypes::IDENTIFIER) {
            Expression::Identifier(self.advance().lexeme.clone())
        } else {
            return Err("Expected column name in WHERE clause".to_string());
        };
...
mod tests {
    ...
    fn test_select_statement() {
        let mut parser = Parser::new("SELECT name, age FROM users WHERE age > 18".to_string());
